import { pgTable, varchar, timestamp, text, integer, index, uniqueIndex, foreignKey, boolean } from "drizzle-orm/pg-core"
...
export const file = pgTable("File", {
	...
	index("File_userId_idx").using("btree", table.userId.asc().nullsLast().op("text_ops")),
	uniqueIndex("File_userId_key").using("btree", table.userId.asc().nullsLast().op("text_ops")),
	foreignKey({
...
export const answerRegis = pgTable("AnswerRegis", {
	id: text().primaryKey().notNull(),
	userId: text().notNull(),
	answer1: text(),
	answer2: text(),
	...
	index("AnswerRegis_userId_idx").using("btree", table.userId.asc().nullsLast().op("text_ops")),
	uniqueIndex("AnswerRegis_userId_key").using("btree", table.userId.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [user.id],
...
export const answerAcademic = pgTable("AnswerAcademic", {
	id: text().primaryKey().notNull(),
	userId: text().notNull(),
	...
	index("AnswerAcademic_userId_idx").using("btree", table.userId.asc().nullsLast().op("text_ops")),
	uniqueIndex("AnswerAcademic_userId_key").using("btree", table.userId.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [user.id],
...
export const user = pgTable("User", {
	...
	uniqueIndex("User_google_id_key").using("btree", table.googleId.asc().nullsLast().op("text_ops")),
]);
